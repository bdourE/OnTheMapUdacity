        ...
        sessionObject.makeRequest(Url: url, requestMethod: method, requestHeaders: finalHeaders, requestBody: body) { (data, error) in
            ...
            if let data = data {
                do {
                    ...
                    let parsedData = try JSONSerialization.jsonObject(with: data.subdata(with: NSMakeRange(5, data.length - 5)), options: []) as! [String:Any]
                    
                    responseClosure(parsedData as [String : AnyObject], nil)
    ...
    func fetchStudentData(fromKey: String, responseClosure: @escaping (_ student: StudentModel?, _ error: String?) -> Void){
        ...
        makeRequestToUdacity(url: studentURL, method: .GET) { (jsonResponseDic, error) in
            ...
            if let jsonResponseDic = jsonResponseDic
                //, let userInformation = jsonResponseDic[JSONResponseKeys.user] as? [String : AnyObject], let firstName = userInformation[JSONResponseKeys.firstName] as? String
                //, let lastName = userInformation[JSONResponseKeys.lastName] as? String
            {
                ...
                var lastName = userInformation![JSONResponseKeys.lastName] as? String
